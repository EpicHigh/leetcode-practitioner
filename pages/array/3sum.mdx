...
The approach I showed earlier, We can sort the input array nums, and then use two pointers to find all unique triplets that sum up to zero. This approach has a time complexity of O(n^2) and a space complexity of O(1).

### Sorting and Binary Search

We can sort the input array nums, and then use binary search to find the complement of each element that makes the sum of three elements equal to zero. This approach has a time complexity of O(n^2 log n) and a space complexity of O(1).
...
function threeSum(nums: number[]): number[][] {
    ...
    for (let i = 0; i < n-2; i++) {
        ...
        for (let j = i+1; j < n-1; j++) {
            ...
            const k = binarySearch(nums, j+1, n-1, complement);
            if (k > j) {
                result.push([nums[i], nums[j], nums[k]]);
            }
        }
